...
export const mappluto = tool({
  ...
  inputSchema: z.object({
    ...
    searchTerm: z
      .string()
      .optional()
      .describe('Search BBL, address, or owner name'),
    ...
    buildingClass: z
      ...
      .optional()
      .describe(
        'Building class (A1=single family, O4=office, R4=condo, V9=vacant, etc.)',
      ),
    landUse: z
      .string()
      .optional()
      .describe('Land use code (01=family homes, 13=office, 29=vacant, etc.)'),
    ownerType: z
      .string()
      .optional()
      .describe('Owner type (P=private, C=city, etc.)'),
    ...
    minAssessment: z
      ...
      .min(0)
      .optional()
      .describe('Minimum total assessment value'),
    maxAssessment: z
      .number()
      .int()
      .min(0)
      .optional()
      .describe('Maximum total assessment value'),
    ...
    minLotArea: z
      ...
      .min(0)
      .optional()
      .describe('Minimum lot area in sq ft'),
    maxLotArea: z
      .number()
      .int()
      .min(0)
      .optional()
      .describe('Maximum lot area in sq ft'),
    minBuildingArea: z
      .number()
      .int()
      .min(0)
      .optional()
      .describe('Minimum building area in sq ft'),
    maxBuildingArea: z
      .number()
      .int()
      .min(0)
      .optional()
      .describe('Maximum building area in sq ft'),
    ...
    maxYearBuilt: z
      ...
      .max(2024)
      .optional()
      .describe('Maximum year built'),
    minUnits: z
      .number()
      .int()
      .min(0)
      .optional()
      .describe('Minimum number of units'),
    maxUnits: z
      .number()
      .int()
      .min(0)
      .optional()
      .describe('Maximum number of units'),
    ...
    hasHighFAR: z
      ...
      .optional()
      .describe(
        'Properties with significant unused FAR (development potential)',
      ),
    ...
    limit: z
      ...
      .max(500)
      .default(100)
      .describe('Maximum number of properties to return'),
    sortBy: z
      .enum(['assessment', 'area', 'yearBuilt', 'units'])
      .default('assessment')
  ...
  execute: async ({
    ...
    try {
      ...
      if (searchTerm) {
        ...
        filteredResults = filteredResults.filter(
          (prop) =>
            ...
            (searchLower.includes('office') &&
              ['O', 'I'].some((prefix) =>
                prop.bldgclass?.startsWith(prefix),
              )) ||
            (searchLower.includes('residential') &&
              ['R', 'A', 'B', 'C', 'D'].some((prefix) =>
                prop.bldgclass?.startsWith(prefix),
              )) ||
            (searchLower.includes('commercial') &&
              ['K', 'M', 'O', 'S'].some((prefix) =>
                prop.bldgclass?.startsWith(prefix),
              )) ||
            (searchLower.includes('building') &&
              prop.bldgclass &&
              !prop.bldgclass.startsWith('V')) ||
            (searchLower.includes('brooklyn') && prop.borough === 'Brooklyn') ||
            (searchLower.includes('vacant') &&
      ...
      if (buildingClass) {
        filteredResults = filteredResults.filter(
          (prop) => prop.bldgclass === buildingClass,
        );
      }

      if (landUse) {
        filteredResults = filteredResults.filter(
          (prop) => prop.landuse === landUse,
        );
      }

      if (ownerType) {
        filteredResults = filteredResults.filter(
          (prop) => prop.ownertype === ownerType,
        );
      }

      if (minAssessment) {
        filteredResults = filteredResults.filter(
          (prop) => (prop.assesstot || 0) >= minAssessment,
        );
      }

      if (maxAssessment) {
        filteredResults = filteredResults.filter(
          (prop) => (prop.assesstot || 0) <= maxAssessment,
        );
      }

      if (isVacant) {
        filteredResults = filteredResults.filter(
          (prop) => prop.bldgclass?.startsWith('V') || prop.landuse === '29',
        );
      }
      ...
      if (sortBy === 'assessment') {
        ...
        filteredResults.sort(
          (a, b) => (b.unitstotal || 0) - (a.unitstotal || 0),
        );
      }
      ...
      const totalAssessedValue = limitedResults.reduce(
        (sum, prop) => sum + (prop.assesstot || 0),
        0,
      );
      const avgAssessment =
        limitedResults.length > 0
          ? totalAssessedValue / limitedResults.length
          : 0;

      const lotsWithArea = limitedResults.filter((p) => p.lotarea);
      const avgLotSize =
        lotsWithArea.length > 0
          ? lotsWithArea.reduce((sum, p) => sum + (p.lotarea || 0), 0) /
            lotsWithArea.length
          : 0;
      ...
      const avgYearBuilt =
        buildingsWithYear.length > 0
          ? buildingsWithYear.reduce((sum, p) => sum + (p.yearbuilt || 0), 0) /
